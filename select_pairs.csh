...
#
  ...
  if ($#argv < 3) then
    echo ""
    echo "Usage: select_pairs.csh baseline_table.dat threshold_time threshold_baseline output.dat"
    echo "generate the input file for intf_tops.csh with given threshold of time and baseline"
    echo "outputs:intf.in. if not specify, use default."    
    echo ""
    exit 1
  endif
...
# loop over possible pairs
  ...
  gmt pstext text -JX8.8i/6.8i -R$region[1]/$region[2]/$region[3]/$region[4] -D0.2/0.2 -X1.5i -Y1i -K -N -F+f8,Helvetica+j5 > baseline_selected.ps  
  ...
  foreach line1 (`awk '{print $1":"$2":"$3":"$4":"$5}' < $file`)
    foreach line2 (`awk '{print $1":"$2":"$3":"$4":"$5}' < $file`)
      ...
      if ($t1 < $t2 & $t2 - $t1 < $dt) then
        ...
        if ($db0 < $db) then
          echo $n1 $n2 | awk '{print $1":"$2}' >> $output
  ...
  gmt psxy text2 -Sp0.2c -G0 -R -JX -Ba0.5:"year":/a50g00f25:"baseline (m)":WSen -O >> baseline_selected.ps
