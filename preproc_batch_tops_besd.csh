    ...
    if($mode == 1) then
      ...
      foreach file (`awk '{print $0}' tmp.filelist`)
	    ...
	    wait
          ...
          # Now start to try modifying it for bivariate enhanced spectral diversity
          # Based on align_tops_esd.csh in GMTSAR
          ...
          gmt blockmedian test -R0/$rmax/0/$amax -I500/100 -r -bo3d > test_b	
   		  ...
   		  # BUG FOUND: KEEP INCREMENT CONSISTENT (-I16/8 or -I8/4)
 		  gmt grdsample test.grd -R0/$rmax/0/$amax -Gtest_b.grd -I8/4 -r -nc
  		  gmt grdmath test_b.grd FLIPUD $spec_sep DIV 2 PI MUL DIV = res_shift.grd
  		  # add shift to azimuth offset, then rename it
